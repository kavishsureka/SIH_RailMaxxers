...
The current implementation is a Next.js 16 frontend, Go 1.23 API, Python/scikit-learn priority model, C++20 optimizer with native OR-Tools CP-SAT support, and optional PostgreSQL 17 benchmark persistence.
...
- one persisted Gradient Boosting priority model, one batch of scores per request, and the same scores for all algorithms;
- shared candidate preprocessing, Independent baseline, coordinated Greedy, and full-month CP-SAT;
- one weighted objective (`wB`, `wD`, `wT`, `wL`, `wV`, `wP`), shared independent validator, derived blocks/traces, and all three runtime fields;
...
- [`../docs/SOLVER_AND_OPTIMIZATION.md`](../docs/SOLVER_AND_OPTIMIZATION.md)
- [`../docs/DATA_SCHEMA_AND_PREPROCESSING.md`](../docs/DATA_SCHEMA_AND_PREPROCESSING.md)
